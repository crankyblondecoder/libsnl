...
// *** NOTE: Some functions still use t,u for surface parameters ***
...
bool newtonIterStepSurf(snlPoint* derivPts, snlPoint* projPt, knot* deltaU, knot* deltaV)
{
	...
	snlVector dist = derivPts[0] - *projPt;  // Distance from point to be projected to surface.

	jcbn[0][0] = derivPts[3].lengthSqrd() + dist.dot(derivPts[6]);
	jcbn[0][1] = snlVector(derivPts[3]).dot(derivPts[1]) + dist.dot(derivPts[4]);
	jcbn[1][0] = jcbn[0][1];
	jcbn[1][1] = derivPts[1].lengthSqrd() + dist.dot(derivPts[2]);

	return solve2X2LinEqn(jcbn[0][0], jcbn[1][0], jcbn[0][1], jcbn[1][1], -( dist.dot(derivPts[3])),
		-( dist.dot(derivPts[1])), deltaU, deltaV);
}
								...
								double iterTol, double cosTol, unsigned maxPass)
{
	...
	for(cPass = 1; cPass <= maxPass; cPass ++)
	{
						...
						for(unsigned sectT = 0; sectT <= (unsigned) pTStepDenom; sectT ++)
						{
							...
							for(unsigned sectU = 0; sectU <= (unsigned) pUStepDenom; sectU ++)
							{
								...
								for(index = 0; index < toProjNum; index ++)
								{
									...
									if((cBest -> pt - toProj[index]).length() > (evalPt - toProj[index]).length())
									{
		...
		for(index = 0; index < toProjNum; index ++)
		{
			...
			double guessDistance = (toProj[index] - cBest -> pt).length();
			...
			while(cBest)
			{
				...
				for(int count = 0; count < MAX_PROJ_ITER; count ++)
				{
					...
					cBest -> dist = (toProj[index] - cBest -> pt).length();
					...
					snlVector projToSurf = evalDerivs[0] - toProj[index];
