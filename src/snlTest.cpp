...
snlCurve* generateCurve()
{
   // Create some points to interpolate.
    ...
    {
        points [ count ].setComponents ( x, y, z, w );
...
snlSurface* generateSurface()
{
    ...
    for ( int tIndex = 0; tIndex < 10; tIndex ++ )
    {
        for ( int uIndex = 0; uIndex < 10; uIndex ++ )
        {
            ...
            points [ ( tIndex * 10 ) + uIndex ].setComponents ( newX, newY, z, w );
...
snlSurface* generateSurface2()
{
    ...
    points [ 0 ].setComponents ( - 2.5, 0.5, z, w );
    points [ 1 ].setComponents ( - 2.0, 1.0 , z, w );
    points [ 2 ].setComponents ( - 1.5, 0.5, z, w );
    points [ 3 ].setComponents ( - 1.0, 0.0, z, w );
    points [ 4 ].setComponents ( - 0.5, 0.5, z, w );
    points [ 5 ].setComponents ( 0.0, 1.0, z, w );
    points [ 6 ].setComponents ( 0.5, 0.5, z, w );
    points [ 7 ].setComponents ( 1.0, 0.0, z, w );
    points [ 8 ].setComponents ( 1.5, 0.5, z, w );
    points [ 9 ].setComponents ( 2.0, 1.0, z, w );
    points [ 10 ].setComponents ( 2.5, 1.5, z, w );
...
snlSurface* generateSawToothSurface()
{
    ...
    for ( int tIndex = 0; tIndex < size; tIndex ++ )
    {
        ...
        {
            points [ ( tIndex * size ) + uIndex ].setComponents ( x + ( xStep * uIndex ), y + ( uIndex % 2 ) * ySaw, z, w );
        }
...
snlSurface* generateSurfaceOfRevolution()
{
    ...
    cpts [ 1 ].setComponents ( 0.0, inlet_radius, ( impeller_depth * 2.0 ) / 3.0, 1.0 );
    cpts [ 2 ].setComponents ( 0.0, inlet_radius, impeller_depth / 3.0, 1.0 );
    cpts [ 3 ].setComponents ( 0.0, ( outlet_radius - inlet_radius ) / 3.0 + inlet_radius, 0.0, 1.0 );
    cpts [ 4 ].setComponents ( 0.0, ( ( outlet_radius - inlet_radius ) * 2.0 ) / 3.0 + inlet_radius, 0.0, 1.0 );


    cpts [ 2 ].weight ( innerProfile_weight1 );
    cpts [ 3 ].weight ( innerProfile_weight2 );

    start -> setComponents ( 0.0, 0.0, impeller_depth, 1.0 );
    end -> setComponents ( 0.0, 0.0, 0.0, 1.0 );
...
snlSurface* generateAmbigSurface()
{
    ...
    line [ 0 ].setComponents ( 0.0, 0.0, 5.0 );
    line [ 1 ].setComponents ( 0.0, 1.0, 1.0 );
    line [ 2 ].setComponents ( 0.0, - 1.0, 1.0 );
    line [ 3 ].setComponents ( 0.0, 0.0, 5.0 );
