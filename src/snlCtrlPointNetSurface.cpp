	...
	snlPoint& cornerX, snlPoint& cornerY)
{
	...
	snlVector O_X = cornerX - origin;
	snlVector O_Y = cornerY - origin;
...
double snlCtrlPointNetSurface::calcFlatness(int indexU, int indexV, int numPointsU, int numPointsV)
{
	...
	snlVector ab = ptB -ptA;
	ab.normalise();

	snlVector ba = ab * -1.0;

	snlVector ac = ptC - ptA;
	ac.normalise();

	snlVector ca = ac * -1.0;

	snlVector bd = ptD - ptB;
	bd.normalise();

	snlVector db = bd * -1.0;

	snlVector cd = ptD - ptC;
	cd.normalise();

	snlVector dc = cd * -1.0;

	// Diagonal Vectors.
	snlVector bc = ptC - ptB;
