...
void snlSurface::fitBilinearCoons(snlPoint* points, int numPoints)
{
	...
	for(int index = 0; index < numPoints; index ++)
	{
		...
		{
			sectionLengths[index] = (normPoints[index + 1] - normPoints[index]).length();
		}
		else
		{
			sectionLengths[index] = (normPoints[0] - normPoints[index]).length(); // Loop join.
		}
...
void snlSurface::fitPlane(snlPoint* points, int numPoints)
{
	...
	{
		double dist = (points[index] - points[0]).lengthSqrd();
	...
	for(int index = 1; index < numPoints; index ++)
	{
		...
		dist = (projPoint - points[baseLineEndIndex]).lengthSqrd();
...
void snlSurface::fitCylinder(snlPoint* points, int numPoints, snlPoint* axisStart, snlPoint* axisEnd)
{
	...
	snlVector axis = *axisEnd - *axisStart;
	...
	for(int index = 0; index < numPoints; index ++)
	{
		...
		if((projPt - newAxisStart).dot(axis) < 0.0)
			newAxisStart = projPt;

		if((projPt - newAxisEnd).dot(axis) > 0.0)
			newAxisEnd = projPt;
...
void snlSurface::fitCone(snlPoint* points, int numPoints, snlPoint* axisStart, snlPoint* axisEnd)
{
	...
	if(intersectsAxis)
	{
		...
		rotation.multiply(angleProjMin);
	}
...
void snlSurface::fitSphere(snlPoint* points, int numPoints, snlPoint* sphereCentre)
{
	...
	snlVector baseVect = points[0] - *sphereCentre;  // Basis vector associated with point 0. Stays constant.
	snlVector refVect = points[1] - *sphereCentre;  // Reference vector. Calculated per point other than point 0.
	...
	latTransf.multiply(startPoint);
	lonTransf.multiply(startPoint);
	...
	latTransf.multiply(endPoint);
	lonTransf.multiply(endPoint);
